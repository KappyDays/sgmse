...
if __name__ == '__main__':
     ...
     # optimizer Adam capturable error 해결
     if "--resume_from_checkpoint" in _:
          model.reTrain = True
     ...
     if args.num_eval_files:
          checkpoint_callback_pesq = ModelCheckpoint(dirpath=f"logs/{logger.version}", 
               save_top_k=3, monitor="pesq", mode="max", filename='{epoch}-{pesq:.2f}')
          checkpoint_callback_si_sdr = ModelCheckpoint(dirpath=f"logs/{logger.version}", 
               save_top_k=3, monitor="si_sdr", mode="max", filename='{epoch}-{si_sdr:.2f}')
