...
class Specs(Dataset):
    def __init__(self, data_dir, subset, dummy, shuffle_spec, num_frames,
        ...
        self.dataset_type = ignored_kwargs["dataset_type"]
        if format == "default":
            # self.clean_files = sorted(glob(join(data_dir, subset) + '/clean/*.wav'))
            self.noisy_files = sorted(glob(join(data_dir, subset) + '/noisy/*.wav'))
            self.clean_files = exec_conf.load_clean_files(self.dataset_type, 
                                                          data_dir=data_dir, 
                                                          subset=subset, 
                                                          noisy_files=self.noisy_files)
        else:
    ...
    ## Gaussian noise를 만들어줌
    def generate_gaussian_noise(self, Gmu, Gsig, size):
        noise = np.random.normal(Gmu, Gsig, size)
    ...
    def __getitem__(self, i):
        ...
        # y = self.generate_gaussian_noise(0, 1, x.shape)
...
class SpecsDataModule(pl.LightningDataModule):
    ...
    def add_argparse_args(parser):
        ...
        # Added argument by me
        parser.add_argument("--dataset_type", type=str, choices=("use_clean_multiple", "use_other_clean_dir", "default"), default="default", help="Choose data type to execute")
        return parser
    ...
    def setup(self, stage=None): # kkr1
        specs_kwargs = dict(
    ...
    def train_dataloader(self): # kkr6
        return DataLoader(
