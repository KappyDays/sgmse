...
# from my_utils.exec_conf import inference_split_size, inference_block_num
...
#         block_size = split_size * inference_block_num # 384
...
#             elif split == True:
#                 temp_Y = []
#                 overlab_block_num = y.size(-1) // block_size # 3
#                 adding_size = y.size(-1) % split_size # 
#                 # pdb.set_trace()
#                 if y.size(-1) // block_size > 0:
#                     for j in range(overlab_block_num):
#                         if j != overlab_block_num-1: # 뒤에서 3번째 block 까지는 그냥 추가
#                             temp_Y.append(y[..., j*block_size:(j+1)*block_size+split_size])
#                         elif j == overlab_block_num-1: # 뒤에서 2번째 block은 따로 추가
#                             if (j+1)*block_size+split_size <= y.size(-1):
#                                 temp_Y.append(y[..., j*block_size:(j+1)*block_size+split_size])
#                             else:
#                                 temp_Y.append(y[..., j*block_size:(j+1)*block_size+adding_size])
#                     if y.size(-1) % block_size != 0: # 나머지 정보 있으면 추가
#                         # left_size = y.size(-1) - (j+1)*block_size
#                         temp_Y.append(y[..., (j+1)*block_size:y.size(-1)])
#                 else:
#                     temp_Y.append(y)

#                 re = torch.Tensor().cuda()
#                 prev_info = torch.zeros(0)
...
#                     # if x_result.size(-1) == block_size+split_size: # split_size만큼 조각이 있는 경우
#                     # pdb.set_trace()
                    
#                     if temp_y.size(-1) > block_size:
#                         signal_list = [] # x_temp, overlab_info1, overlab_info2
#                         signal_list.append(x_result[..., :block_size])
#                         signal_list.append(x_result[..., block_size-split_size//2:block_size])
#                         # signal_list.append(x_result[..., block_size:block_size+split_size//2])
                        
#                         ## istft를 통해 음성을 얻고 overlab함.
#                         sig_result = []
#                         windows = [torch.hann_window(254), torch.hamming_window(254)] #, torch.blackman_window(254)]
#                         for signal in signal_list:
#                             temp = score_fn.data_module.spec_back(signal)
#                             sig_result.append(torch.istft(temp.squeeze(), n_fft=254, hop_length=128, 
#                                                         window=windows.pop().to(temp.device), 
#                                                         center=True))
                            
#                         current_wav = sig_result[0]
#                         current_wav[..., :prev_info.size(0)] += prev_info.to(current_wav.device)
                        
#                         prev_info = sig_result[1]
#                     else:
