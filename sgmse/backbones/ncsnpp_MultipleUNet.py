...
@BackboneRegistry.register("ncsnpp_multipleunet")
class NCSNpp_MultipleUNet(nn.Module):
    ...
    def add_argparse_args(parser):
        ...
        # 채널 args 추가할것?
        parser.add_argument("--nf", type=int, default=64, help="Channels of First Layer")
        parser.add_argument("--TwoCross", type=bool, default=False, help="Two CrossAttn or Shared CrossAttn")
        parser.add_argument("--cross_attn_resolutions", type=int, nargs='+', default=[32])
        return parser

    def __init__(self,
        scale_by_sigma = True,
        nonlinearity = 'swish',
        nf = 64, # 시작 채널, default=128
        ch_mult = (1, 1, 2, 2, 2, 2, 2), # nf * ch_mult하면 각 layer의 channel
        num_res_blocks = 2,
        attn_resolutions = (16,),
        cross_attn_resolutions = (32, ),
        ...
        **unused_kwargs
        ...
        self.TwoCross = TwoCross = TwoCross = False

        # num_channels = 4  # x.real, x.imag, y.real, y.imag
        num_channels = 2  # 4차원 입력을 2차원 2개로 나눔
        # 최종 output은 concat된 차원
        output_layer_ch = 4
        self.output_layer = nn.Conv2d(output_layer_ch, 2, 1)
        ...
        if TwoCross == True:
            CrossAttnblock = functools.partial(layerspp.TwoCrossAttnBlockpp,
                                            init_scale=init_scale, cross=True, skip_rescale=skip_rescale)
        else:
        ...
        for i_level in range(num_resolutions):
            ...
            for i_block in range(num_res_blocks):
                ...
                if all_resolutions[i_level] in cross_attn_resolutions:
                    modules.append(CrossAttnblock(channels=in_ch))
                
                hs_c.append(in_ch)
                
            if i_level != num_resolutions - 1:
                if resblock_type == 'ddpm': # default: biggan
                    modules.append(Downsample(in_ch=in_ch))
                else: 
                    modules.append(ResnetBlock(down=True, in_ch=in_ch)) # Resolution Down
    ...
    def forward(self, x, time_cond):
        ...
        for i_level in range(self.num_resolutions):
            ...
            for i_block in range(self.num_res_blocks):
                ...
                h_n = modules_n[m_idx](hn[-1], temb)
                m_idx += 1
                # Attention layer (optional)
                if h_s.shape[-2] in self.attn_resolutions: # edit: check H dim (-2) not W dim (-1)
                    h_s = modules_s[m_idx](h_s)
                    h_n = modules_n[m_idx](h_n)
                    m_idx += 1
                # Encoding cross-attention layer
                if h_s.shape[-2] in self.cross_attn_resolutions: #cross attention
                    if self.TwoCross == True: # 각각의 module에 있는 attn 사용
                        cross_attn_in_qs_vn = torch.cat((h_s, h_n), dim=1) # h_s가 query, h_n이 key, value
                        cross_attn_in_qn_vs = torch.cat((h_n, h_s), dim=1) # h_n가 query, h_s이 key, value
                        h_n = modules_s[m_idx](cross_attn_in_qs_vn) 
                        h_s = modules_n[m_idx](cross_attn_in_qn_vs)
                        m_idx += 1
                    else: # 한 module의 attn만 사용 (attn 공유)
                        cross_attn_in = torch.cat((h_s, h_n), dim=1)
                        h_s, h_n = modules_s[m_idx](cross_attn_in)
                        m_idx += 1     
                hs.append(h_s)
                hn.append(h_n)
            ...
            if i_level != self.num_resolutions - 1:
                ...
                if self.progressive_input == 'input_skip':   # Combine h with x
                    ...
                    input_pyramid_n = self.pyramid_downsample(input_pyramid_n)
                    h_s = modules_s[m_idx](input_pyramid_s, h_s)
                    h_n = modules_n[m_idx](input_pyramid_n, h_n)
                    m_idx += 1

                elif self.progressive_input == 'residual':
                    input_pyramid_s = modules_s[m_idx](input_pyramid_s)
                    input_pyramid_n = modules_n[m_idx](input_pyramid_n)
                    m_idx += 1
                    if self.skip_rescale:
                        input_pyramid_s = (input_pyramid_s + h_s) / np.sqrt(2.)
                        input_pyramid_n = (input_pyramid_n + h_n) / np.sqrt(2.)
                    else:
                        input_pyramid_s = input_pyramid_s + h_s
                        input_pyramid_n = input_pyramid_n + h_n
                    h_s = input_pyramid_s
                    h_n = input_pyramid_n
        ...
        h_n = hn[-1] # actualy equal to: h = h
        h_s = modules_s[m_idx](h_s, temb)  # ResNet block
        h_n = modules_n[m_idx](h_n, temb)  # ResNet block
        m_idx += 1
        h_s = modules_s[m_idx](h_s)  # Attention block
        h_n = modules_n[m_idx](h_n)  # Attention block
        m_idx += 1
        h_s = modules_s[m_idx](h_s, temb)  # ResNet block
        h_n = modules_n[m_idx](h_n, temb)  # ResNet block
        m_idx += 1
        ...
        for i_level in reversed(range(self.num_resolutions)):
            for i_block in range(self.num_res_blocks + 1):
                ...
                h_n = modules_n[m_idx](torch.cat([h_n, hn.pop()], dim=1), temb)
                m_idx += 1
            ...
            if h_s.shape[-2] in self.attn_resolutions:
                ...
                h_n = modules_n[m_idx](h_n)
                m_idx += 1
            # Decoding cross-attention layer
            if h_s.shape[-2] in self.cross_attn_resolutions: # cross attention
                if self.TwoCross == True: 
                    cross_attn_in_qs_vn = torch.cat((h_s, h_n), dim=1) # h_s가 query, h_n이 key, value
                    cross_attn_in_qn_vs = torch.cat((h_n, h_s), dim=1) # h_n가 query, h_s이 key, value
                    h_n = modules_s[m_idx](cross_attn_in_qs_vn)
                    h_s = modules_n[m_idx](cross_attn_in_qn_vs)
            ...
            if self.progressive != 'none':
                if i_level == self.num_resolutions - 1: # 첫 layer인 경우 최초 progressive input 설정
                    if self.progressive == 'output_skip':
                        ...
                        pyramid_n = self.act(modules_n[m_idx](h_n))  # GroupNorm
                        m_idx += 1
                        pyramid_s = modules_s[m_idx](pyramid_s)  # Conv2D: 256 -> 4
                        pyramid_n = modules_n[m_idx](pyramid_n)  # Conv2D: 256 -> 4
                        m_idx += 1
                    elif self.progressive == 'residual':
                        pyramid_s = self.act(modules_s[m_idx](h_s))
                        pyramid_n = self.act(modules_n[m_idx](h_n))
                        m_idx += 1
                        pyramid_s = modules_s[m_idx](pyramid_s)
                        pyramid_n = modules_n[m_idx](pyramid_n)
                        m_idx += 1
                    else:
                    ...
                    if self.progressive == 'output_skip':
                        ...
                        pyramid_n = self.pyramid_upsample(pyramid_n)  # Upsample
                        pyramid_hs = self.act(modules_s[m_idx](h_s))  # GroupNorm
                        pyramid_hn = self.act(modules_n[m_idx](h_n))  # GroupNorm
                        m_idx += 1
                        pyramid_hs = modules_s[m_idx](pyramid_hs)
                        pyramid_hn = modules_n[m_idx](pyramid_hn)
                        m_idx += 1
                        pyramid_s = pyramid_s + pyramid_hs
                        pyramid_n = pyramid_n + pyramid_hn
                    elif self.progressive == 'residual':
                        pyramid_s = modules_s[m_idx](pyramid_s)
                        pyramid_n = modules_n[m_idx](pyramid_n)
                        m_idx += 1
                        if self.skip_rescale:
                            ...
                            pyramid_n = (pyramid_n + h_n) / np.sqrt(2.)
                        else:
                            pyramid_s = pyramid_s + h_s
                            pyramid_n = pyramid_n + h_n
                        h_s = pyramid_s
                        h_n = pyramid_n
                    else:
                        raise ValueError(f'{self.progressive} is not a valid name')
            ...
            if i_level != 0:
                if self.resblock_type == 'ddpm':
                    ...
                    h_n = modules_n[m_idx](h_n, temb)  # Upspampling
                    m_idx += 1
        ...
        if self.progressive == 'output_skip':
            ...
            h_n = self.act(modules_n[m_idx](h_n))
            m_idx += 1
            h_s = modules_s[m_idx](h_s)
            h_n = modules_n[m_idx](h_n)
            m_idx += 1
        ...
        assert m_idx == len(modules_n), "Implementation error n"
