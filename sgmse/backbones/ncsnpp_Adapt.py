...
@BackboneRegistry.register("ncsnpp_decattn")
class NCSNpp_DeCAttn(nn.Module):
    ...
    def add_argparse_args(parser):
        ...
        parser.add_argument("--unet_cross_attn", type=str, default="False")
        parser.add_argument("--cd_block", type=str, default="False")
    ...
    def __init__(self,
        ...
        **unused_kwargs
        ...
        ############################# My Args
        self.prog_attn_resolutions = prog_attn_resolutions
        self.DeCAttn_loss = False
        self.ProgDeCAttn_loss = False
        self.unet_cross_attn = unet_cross_attn
        self.cd_block = cd_block
        self.cd_block_resolutions = cd_block_resolutions
        ##############################
        ...
        if self.unet_cross_attn == 'True':
            print("UNet Cross Attn 추가요")
            modules.append(CrossAttnBlock(channels=num_channels//2, loss=False))
        ##########################################################
        
        ################# 첫 단에 CD block 붙임 #################
        if all_resolutions[0] in cd_block_resolutions:
    ...
    def forward(self, x, time_cond):
        ...
        ##################
        ################# 첫 단에 CD block 붙임 #################
        if x.shape[-2] in self.cd_block_resolutions:
            x = modules_s[m_idx](x)
            m_idx += 1            
        ###########################################################                    
